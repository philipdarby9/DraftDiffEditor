...
Use `File` -> `Backup folder` to choose or create a shared backup/history folder. Version-history JSON sidecars are written under that folder's `json` subfolder, named like `draft-history.version-history.json`. Normal editing autosaves keep the canonical sidecar current without creating an immutable JSON backup; a snapshot in `version history JSON backups` is created when the app closes. When a text file is opened, the app looks for a matching sidecar by source path metadata or by filename, then merges those histories into the matching Project notes, draft pages, and draft notes.
